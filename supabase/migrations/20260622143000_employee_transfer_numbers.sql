create table if not exists public.employee_transfer_counters (
  company_id uuid primary key references public.companies(id) on delete cascade,
  next_transfer_number bigint not null default 1
);

alter table public.employee_transfers
  add column if not exists transfer_number bigint null;

create unique index if not exists employee_transfers_company_transfer_number_unique
  on public.employee_transfers(company_id, transfer_number);
...
with numbered_transfers as (
  select
    ...
    row_number() over (
      ...
      order by transfer_date asc, transfer_time asc, created_at asc, id asc
    ) as transfer_number
  from public.employee_transfers
)
update public.employee_transfers et
set transfer_number = numbered_transfers.transfer_number
from numbered_transfers
where et.id = numbered_transfers.id
  and et.transfer_number is null;

insert into public.employee_transfer_counters (company_id, next_transfer_number)
select
  company_id,
  coalesce(max(transfer_number), 0) + 1
from public.employee_transfers
group by company_id
on conflict (company_id) do update
set next_transfer_number = excluded.next_transfer_number;
...
begin
  ...
  from public.employee_transfer_counters
  where company_id = p_company_id
  for update;

  if not found then
    raise exception 'No se pudo reservar el número de traspaso.';
  end if;

  update public.employee_transfer_counters
  set next_transfer_number = next_transfer_number + 1
  where company_id = p_company_id;
